...
class CharacterIdentifier:
    def __init__(self):
        ...
        self.label_dict_reverse = {0: 'tracer', 1: 'other'}
    ...
    def apply_transformation(self, img):
        ...
        # Resize
        img = cv2.resize(img, (round(1920*self.downsize_proportion), round(1080*self.downsize_proportion)))
        # Show (Temp)
        # cv2.imshow('img', img)
        # cv2.waitKey(0)
...
class TrainModel:
    def __init__(self, data_path = 'data/', train_test_split = 0.8, max_each_class = 2800):
        self.model = None
        ...
        # Downsize Proportion
        self.downsize_proportion = 0.5
        
        self.classes = [0, 1]
        self.label_dict = {'tracer': 0, 'other': 1}
    ...
    def train(self):
        ...
        # # Logistic Regression
        # self.model = LogisticRegression(solver='lbfgs', multi_class='multinomial', max_iter=10000, verbose=1)
        # SGD Classifier
        # self.model = SGDClassifier(loss='log', max_iter=10000, verbose=1)
        # SVM Classifier
        self.model = LinearSVC(max_iter=10000, verbose=1)
        # Fit Data
...
class GatherData:
    def __init__(self, save_folder = f'data', character_name = 'other', resolution = (1920, 1080), time_between_frames = 0.1, start_file_number = 0):
        # Member variables
